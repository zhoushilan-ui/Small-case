...
(function () {
  function q(a) {
    ...
    m && e
      ? ((a = Math.max.apply(null, [m.r, m.g, m.b])),
        (m = Math.max.apply(null, [e.r, e.g, e.b])),
        (m = Math.round(m + -0.75 * (m - a))),
        (m = { r: m, g: m, b: m }))
      : m
  ...
  function n(a, b, c, g) {
    if ("ratio" === b || "%" === b.split("")[b.length - 1])
      this.eventTokens[c + ".parentresized"] = g.subscribe(
        c + ".parentresized",
        (function (b, m, e, l) {
          return function () {
            var d = m.width();
            if (d !== e) {
              for (var l in b)
                b.hasOwnProperty(l) && (g.unsubscribe(b[l]), delete b[l]);
              var f = a.settings;
              ...
              for (D = l.length; h < D; h++) {
                ...
                for (E = k.x.length; p < E; p++)
                  n.x.push(k.x[p] * d), n.y.push(k.y[p] * d);
                r.push(n);
              }
              f.data = r;
              m[c](f);
  ...
  function w(a, b, c) {
    ...
    g = function (a) {
      var b,
        ...
        d = function (d) {
          d =
            d.changedTouches && 0 < d.changedTouches.length
              ? d.changedTouches[0]
              : d;
          return new h(
            Math.round(d.pageX + b),
            Math.round(d.pageY + c) + a.fatFingerCompensation
          );
        },
      ...
      this.drawMoveHandler = function (b) {
        a.settings.readOnly ||
          (b.preventDefault(),
          a.dataEngine.inStroke && (a.dataEngine.addToStroke(d(b)), g.kick()));
      };
      return this;
    }.call({}, this);
    (function (a, b, c) {
      ...
      if (this.isCanvasEmulator)
        ...
        m
          ...
          : (d.ontouchstart = ontouchstart);
        d.onmousedown = function (g) {
          m
            ? d.removeEventListener("touchstart", this.ontouchstart)
    ...
    a["jSignature.windowmouseup"] = d.subscribe(
  ...
  function x(a) {
    ...
    var b = a.ownerDocument.parentWindow,
      c = b.FlashCanvas
        ? a.ownerDocument.parentWindow.FlashCanvas
      ...
      if (1 !== c)
        try {
          $(a)
            .children("object")
            .get(0)
            .resize(Math.ceil(a.width * c), Math.ceil(a.height * c)),
    ...
    f = function (a, b) {
      ...
      if (1 < b) {
        ...
        if (f.getLength() > this.lineCurveThreshold) {
          var l =
            2 < b
              ? new h(a.x[b - 3], a.y[b - 3]).getVectorToPoint(e)
              : new k(0, 0);
          var n = 0.35 * f.getLength(),
            t = f.angleTo(l.reverse()),
            p = d.angleTo(f.reverse());
          l = new k(l.x + f.x, l.y + f.y).resizeTo(Math.max(0.05, t) * n);
          var r = new k(f.x + d.x, f.y + d.y)
            .reverse()
            .resizeTo(Math.max(0.05, p) * n);
      ...
      d.getLength() <= this.lineCurveThreshold &&
        y(this.canvasContext, c.x, c.y, g.x, g.y);
    },
    e = function (a) {
      var b = a.x.length - 1;
      if (0 < b) {
  ...
  w.prototype.resetCanvas = function (a, b) {
    ...
    if (g.signatureLine) {
      ...
      m ||
        ((d.shadowColor = d.strokeStyle),
        (d.shadowOffsetX = 0.5 * d.lineWidth),
        (d.shadowOffsetY = -0.6 * d.lineWidth),
        (d.shadowBlur = 0));
    }
  ...
  w.prototype.initializeCanvas = function (a) {
    ...
    "ratio" === a.height
      ? c.css("height", Math.round(c.width() / a.sizeRatio))
      : "ratio" === a.width &&
        c.css("width", Math.round(c.height() * a.sizeRatio));
            ...
            if (!k(this)) {
              ...
              if (a) {
                ...
                for (var b in a.eventTokens)
                  a.eventTokens.hasOwnProperty(b) &&
...
(function () {
  for (
    var q = {},
      k = {},
      h = "0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWX".split(
        ""
      ),
      p = h.length / 2,
      n = p - 1;
  ...
  var w = function (e) {
    ...
    v = function (e) {
      ...
      for (var b = e.length, c, g = 1, d = [], f = 0, h = 0; h < b; h++)
        (c = e[h]),
          c in q || "Z" === c || "Y" === c
            ? (0 !== d.length &&
                ((d = parseInt(d.join(""), p) * g + f), a.push(d), (f = d)),
              "Z" === c
                ? ((g = -1), (d = []))
                : "Y" === c
                ? ((g = 1), (d = []))
    ...
    u = function (e) {
      for (var a = [], b = e.length, c, g = 0; g < b; g++)
        (c = e[g]), a.push(x(c.x)), a.push(x(c.y));
      return a.join("_");
    },
    y = function (e) {
      var a = [];
    ...
    f = function (e, a, b) {
      "string" === typeof e &&
        ("image/jsignature;base30" === e.substring(0, 23).toLowerCase() &&
          (e = e.substring(24)),
        b(y(e)));
    };
  if (null == this.jQuery)
...
(function () {
  ...
  function p(f, e, a) {
    ...
    return b.getLength() > a
      ? ((a =
          2 < e
            ? new k(f.x[e - 3], f.y[e - 3]).getVectorToPoint(g)
            : new q(0, 0)),
        (f = 0.35 * b.getLength()),
        (g = b.angleTo(a.reverse())),
        (e = c.angleTo(b.reverse())),
        (a = new q(a.x + b.x, a.y + b.y).resizeTo(Math.max(0.05, g) * f)),
        (c = new q(b.x + c.x, b.y + c.y)
          .reverse()
          .resizeTo(Math.max(0.05, e) * f)),
        (c = new q(b.x + c.x, b.y + c.y)),
        ["c", h(a.x, 2), h(a.y, 2), h(c.x, 2), h(c.y, 2), h(b.x, 2), h(b.y, 2)])
      : ["l", h(b.x, 2), h(b.y, 2)];
  }
  function n(f, e) {
    var a = f.x.length - 1,
    ...
    if (1 < a && b.getLength() > e) {
      ...
      var g = 0.35 * b.getLength();
      a = new q(a.x + b.x, a.y + b.y).resizeTo(Math.max(0.05, c) * g);
      return [
        "c",
        h(a.x, 2),
        h(a.y, 2),
  ...
  function w(f, e, a) {
    ...
    0 < b
      ? e.push.apply(e, n(f, a, 1))
      : 0 === b && e.push.apply(e, ["l", 1, 1]);
    return e.join(" ");
  }
  function x(f) {
    for (
  ...
  function v(f, e) {
    ...
    for (c = p.length; b < c; b++)
      (g = p[b]), a.push("<path " + x(e) + ' d="' + w(g, k, l) + '"/>');
    a.push("</svg>");
    return a.join("");
  }
  function u(f, e) {
